...
retry:
	...
	//if (likely(vma->vm_start <= address))				// Sangjin Luma
	//	goto good_area;						// Sangjin Luma

	/*
	 * Ok, we have a good vm_area for this memory access, so
	 * we can handle it..
	 */
//good_area:								// Sangjin Luma
	if (error_code & X86_PF_WRITE) {				// Sangjin Luma
		if (vma->original_write && vma->vm_ops			// Sangjin Luma
					&& vma->vm_ops->dax_cow) {	// Sangjin Luma
			mmap_read_lock(mm);				// Sangjin Luma
			vma->vm_ops->dax_cow(vma, address);		// Sangjin Luma
			mmap_read_unlock(mm);				// Sangjin Luma
		}							// Sangjin Luma
	}								// Sangjin Luma
