...
#include <linux/page-flags.h>		// Sangjin Luma
#include "nova.h"
#include "inode.h"
...
/* find_get_entries_tag() is not supported anymore
 * copy internal code lines from find_get_entries_tag()
 * Sangjin Luma
 */
static unsigned find_get_entries_tag(struct address_space *mapping, pgoff_t start,
				xa_mark_t tag, unsigned int nr_entries,
				//struct page ** entries, pgoff_t *indices)
				struct folio_batch *fbatch, pgoff_t *indices)	// Sangjin Luma
{
	XA_STATE(xas, &mapping->i_pages, start);
	struct page *page;
	struct folio *folio;				// Sangjin Luma
	unsigned int ret = 0;
	...
	xas_for_each_marked(&xas, folio, ULONG_MAX, tag) {		// Sangjin Luma
		struct page *head;
		//if (xas_retry(&xas, page))
		if (xas_retry(&xas, page_folio(page)))		// Sangjin Luma
			continue;
		/*
		 ...
		 */
		//if (xa_is_value(page))
		if (xa_is_value(page_folio(page)))			// Sangjin Luma
			goto export;

		head = compound_head(page);
		//if (!page_cache_get_speculative(head))
		if (!folio_try_get_rcu(page_folio(head)))		// Sangjin Luma
			goto retry;
...
export:
		...
		indices[fbatch->nr] = xas.xa_index;			// Sangjin Luma
		//entries[ret] = page;
		//if (++ret == nr_entries)
		//	break;
		if(!folio_batch_add(fbatch, folio))				// Sangjin Luma
			break;
		continue;
put_page:
		put_page(head);
	...
	unsigned long start_pgoff, unsigned long end_pgoff)
{
	...
	//struct pagevec pvec;
	struct folio_batch fbatch;		// Sangjin Luma
	bool done = false;
	int count = 0;
	...
	while (!done) {
		...
		//		pvec.pages, indices);
		fbatch.nr = find_get_entries_tag(mapping, start_pgoff,	// Sangjin Luma
				PAGECACHE_TAG_DIRTY, PAGEVEC_SIZE,
				&fbatch, indices);		// Sangjin Luma

		//if (pvec.nr == 0)
		if (fbatch.nr == 0)					// Sangjin Luma
			break;
		...
		//for (i = 0; i < pvec.nr; i++) {
		for (i = 0; i <fbatch.nr; i++) {			// Sangjin Luma
		...
		//count += pvec.nr;
		count += fbatch.nr;					// Sangjin Luma
		//if (pvec.nr < PAGEVEC_SIZE)
		if (fbatch.nr < PAGEVEC_SIZE)				// Sangjin Luma
			break;

		//start_pgoff = indices[pvec.nr - 1] + 1;
		start_pgoff = indices[fbatch.nr - 1] + 1;		// Sangjin Luma
	}
