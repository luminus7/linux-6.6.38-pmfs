...
EXPORT_SYMBOL(vfs_read);	// Sangjin Luma
...
EXPORT_SYMBOL(vfs_write);	// Sangjin Luma
...
ssize_t read_internal(unsigned int fd, char __user * buf, size_t count)
{
	return ksys_read(fd, buf, count);
}
EXPORT_SYMBOL(read_internal);	// Sangjin Luma

SYSCALL_DEFINE3(read, unsigned int, fd, char __user *, buf, size_t, count)
{
	return read_internal(fd, buf, count);	// Sangjin Luma
}
...
ssize_t write_internal(unsigned int fd, const char __user * buf,
		size_t count)
{
	return ksys_write(fd, buf, count);
}
EXPORT_SYMBOL(write_internal);

SYSCALL_DEFINE3(write, unsigned int, fd, const char __user *, buf,
		size_t, count)
{
	return write_internal(fd, buf, count);	// Sangjin Luma
}
