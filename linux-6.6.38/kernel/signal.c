...
int signal_internal(int sig, __sighandler_t handler)		// Sangjin Luma
{
	struct k_sigaction new_sa, old_sa;
	int ret;
...
EXPORT_SYMBOL(signal_internal);		// Sangjin Luma

SYSCALL_DEFINE2(signal, int, sig, __sighandler_t, handler)		// Sangjin Luma
{
	return signal_internal(sig, handler);
}
