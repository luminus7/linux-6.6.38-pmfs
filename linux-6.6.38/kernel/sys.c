...
uid_t getuid_internal(void)
{
	/* Only we change this so SMP safe */
	return from_kuid_munged(current_user_ns(), current_uid());
}
EXPORT_SYMBOL(getuid_internal);		// Sangjin Luma

SYSCALL_DEFINE0(getuid)
{
	return getuid_internal();
}

//SYSCALL_DEFINE0(geteuid)
uid_t geteuid_internal(void)
{
	/* Only we change this so SMP safe */
	return from_kuid_munged(current_user_ns(), current_euid());
}
EXPORT_SYMBOL(geteuid_internal);	// Sangjin Luma

SYSCALL_DEFINE0(geteuid)
{
	return geteuid_internal();
}

//SYSCALL_DEFINE0(getgid)
gid_t getgid_internal(void)
{
	/* Only we change this so SMP safe */
	return from_kgid_munged(current_user_ns(), current_gid());
}
EXPORT_SYMBOL(getgid_internal);

SYSCALL_DEFINE0(getgid)
{
	return getgid_internal();
}

//SYSCALL_DEFINE0(getegid)
gid_t getegid_internal(void)
{
	/* Only we change this so SMP safe */
	return from_kgid_munged(current_user_ns(), current_egid());
}
EXPORT_SYMBOL(getegid_internal);

SYSCALL_DEFINE0(getegid)
{
	return getegid_internal();
}
