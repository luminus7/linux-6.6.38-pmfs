	...
	struct pmfs_direntry *de, u8 *blk_base,  struct pmfs_inode *pidir)
{
	...
	dir->i_mtime = inode_set_ctime_current(inode);			// Sangjin Luma
	/*dir->i_version++; */
	...
	pidir->i_ctime = cpu_to_le32(inode_get_ctime_sec(dir));		// Sangjin Luma
		...
		struct inode *inode)
{
	...
	dir->i_mtime = inode_set_ctime_current(dir);			// Sangjin Luma
	...
	pidir->i_mtime = cpu_to_le32(dir->i_mtime.tv_sec);
	//pidir->i_ctime = cpu_to_le32(dir->i_ctime.tv_sec);
	pidir->i_ctime = cpu_to_le32(inode_get_ctime_sec(inode));	// Sangjin Luma
...
const struct file_operations pmfs_dir_operations = {
	...
	.iterate_shared	= pmfs_readdir,		// Sangjin Luma
	.fsync		= noop_fsync,
