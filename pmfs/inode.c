	...
	struct pmfs_inode *pi, unsigned long newsize, __le64 newroot)
{
	...
	__le64 *root;
	char b[8];
	static DEFINE_SPINLOCK(pmfs_btree_lock);	// Sangjin Luma
...
update_root_and_height:
	...
	*(u64 *)b = *(u64 *)pi;
	...
	// Sangjin Luma
	spin_lock(&pmfs_btree_lock);
	if (cmpxchg64_local((u64 *)pi, *(u64 *)pi, *(u64 *)b)) {
		pr_info("%s(): cmpxchg64_local() on pi success\n", __func__);
		if (cmpxchg64_local(&pi->root, pi->root, newroot)) {
			pr_info("%s(): cmpxchg64_local() on pi->root success\n", __func__);
		} else {
			pr_info("%s(): cmpxchg64_local() on pi->root failed\n", __func__);
		}
	} else {
		pr_info("%s(): cmpxchg64_local() on pi failed\n", __func__);
	}
	spin_unlock(&pmfs_btree_lock);
	// Sangjin Luma end
}
...
static int pmfs_read_inode(struct inode *inode, struct pmfs_inode *pi)
{
	...
	inode_set_ctime(inode, le32_to_cpu(pi->i_ctime), 0);		// Sangjin Luma

	inode->i_mtime.tv_sec = le32_to_cpu(pi->i_mtime);
	//inode->i_atime.tv_nsec = inode->i_mtime.tv_nsec =
	//				 inode->i_ctime.tv_nsec = 0;
	inode->i_atime.tv_nsec = inode->i_mtime.tv_nsec = 0;		// Sangjin Luma
		...
		umode_t mode, const struct qstr *qstr)
{
	...
	inode_init_owner(&nop_mnt_idmap, inode, dir, mode);	// Sangjin Luma
	inode->i_blocks = inode->i_size = 0;
	//inode->i_mtime = inode->i_atime = inode->i_ctime = current_time(inode);
	inode->i_mtime = inode->i_atime = inode_set_ctime_current(inode);	// Sangjin Luma
...
int pmfs_getattr(struct mnt_idmap *idmap, const struct path *path,	// Sangjin Luma
		 struct kstat *stat, u32 request_mask, unsigned int query_flags)
{
	struct inode *inode;
...
//		       struct iattr *attr)
int pmfs_notify_change(struct mnt_idmap *idmap, struct dentry *dentry,	// Sangjin Luma
		       struct iattr *attr)
{
	...
	ret = setattr_prepare(&nop_mnt_idmap, dentry, attr);		// Sangjin Luma
	if (ret)
		return ret;
	...
	setattr_copy(&nop_mnt_idmap, inode, attr);		// Sangjin Luma
