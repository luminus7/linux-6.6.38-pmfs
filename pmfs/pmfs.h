...
extern int pmfs_notify_change(struct mnt_idmap *idmap,				// Sangjin Luma
			      struct dentry *dentry, struct iattr *attr);
	...
	struct pmfs_inode *pi)
{
	...
	static DEFINE_SPINLOCK(pmfs_lock);	// Sangjin Luma
	...
	words[0] = cpu_to_le32(inode_get_ctime_sec(inode));	// Sangjin Luma
	...
	// Sangjin Luma
	spin_lock(&pmfs_lock);
	if (cmpxchg64_local(&pi->i_size, pi->i_size, new_pi_size)) {
		pr_info("%s(): cmpxchg64_local() on i_size success\n", __func__);
		if (cmpxchg64_local((u64 *)&pi->i_ctime, *(u64 *)&pi->i_ctime, *(u64 *)words)) {
			pr_info("%s(): cmpxchg64_local() on i_ctime success\n", __func__);
		} else {
			pr_info("%s(): cmpxchg64_local() on i_ctime failed\n", __func__);
		}
	} else {
		pr_info("%s(): cmpxchg64_local() on i_size failed\n", __func__);
	}
	spin_unlock(&pmfs_lock);
	// Sangjin Luma end
}
